...
type TodoClient interface {
	...
	// Pass a zero-value TodoFilter to list all todos.
	ListTodos(ctx context.Context, filter domain.TodoFilter) ([]domain.Todo, error)
	...
	// Returns domain.ErrNotFound if the todo does not exist.
	UpdateTodo(ctx context.Context, id int64, todo *domain.Todo) (*domain.Todo, error)
	...
	// Returns domain.ErrNotFound if the project does not exist.
	GetProjectTodos(ctx context.Context, projectID int64, filter domain.TodoFilter) ([]domain.Todo, error)
}
